...
public:
    ...
    // Constructor for stereo cameras, with line feature (rectified horizontal stereo).
    Frame(const cv::Mat &imLeft, const cv::Mat &imRight, const cv::Mat &imRGB, const cv::Mat &imRightRGB, const double &timeStamp, ORBextractor* extractorLeft, ORBextractor* extractorRight, LSDextractor* lsdExtractorLeft, LSDextractor* lsdExtractorRight, ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth, GeometricCamera* pCamera,Frame* pPrevF = static_cast<Frame*>(NULL), const IMU::Calib &ImuCalib = IMU::Calib());
    ...
    std::vector<std::size_t> GetLinesInRegionNew(const float &x1, const float &y1, 
                                       const float &x2, const float &y2, 
                                       const float &r) const;
