...
namespace ORB_SLAM3
{
    ...
    public:
        ...
        //int SearchByProjection(KeyFrame *pKF, cv::Mat Scw, const std::vector<MapLine *> &vpLines, std::vector<MapLine *> &vpMatched, int th);
        int SearchForInitialization(Frame &F1, Frame &F2, std::vector<cv::Point2f> &vbPrevMatched, std::vector<int> &vnMatches12, int windowSize);
        ...
        void ComputeThreeMaximaNew(std::vector<int>* histo, const int L, int &ind1, int &ind2, int &ind3);
