...
public:
    ...
    Sophus::SE3f GrabImageMonocularWithLine(const cv::Mat &im, const double &timestamp, string filename);
...
public:
    ...
    //std::vector<cv::Point2f> mvbPrevLineMatched;
    //std::vector<cv::line_descriptor::KeyLine> mvIniKeyLines;
    //std::vector<std::vector<cv::DMatch>> mvIniLineMatches;
    //std::vector<Eigen::Vector3f> mvIniLineColorRGB;
    //std::vector<int> mvIniLineMatches;

    std::vector<cv::Point2f> mvbPrevLineMatched; // 记录线段滑动窗口追踪的基准坐标
    std::vector<int> mvIniLineMatches;           // 记录F1到F2的线段匹配索引
    std::vector<Eigen::Vector3f> mvIniLineColorRGB; // 记录地图线段初始化的颜色
...
protected:
    ...
    void CreateInitialMapMonocularWithLine();
