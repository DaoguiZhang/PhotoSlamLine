...
bool KeyFrame::UnprojectStereoLineSeg(
    const int &i,
    std::pair<Eigen::Vector3f, Eigen::Vector3f> &xLine3D,
    std::pair<Eigen::Vector3f, Eigen::Vector3f> &colorLine3DRGB)
{
    float z1, z2;
    cv::line_descriptor::KeyLine klUn, kl;
    ...
    // 1. 线程安全地获取线特征数据和深度
    {
        ...
        z1 = mvLineDepth[i].first;
        z2 = mvLineDepth[i].second;
        klUn = mvKeyLinesUn[i];
        kl = mvKeyLines[i];
    }
    ...
    // 2. 像素到相机坐标系
    const float x1 = (klUn.startPointX - cx) * z1 * invfx;
    const float y1 = (klUn.startPointY - cy) * z1 * invfy;
    const float x2 = (klUn.endPointX - cx) * z2 * invfx;
    const float y2 = (klUn.endPointY - cy) * z2 * invfy;

    Eigen::Vector3f Xc1(x1, y1, z1);
    Eigen::Vector3f Xc2(x2, y2, z2);

    // 3. 线程安全地获取位姿，相机坐标系 -> 世界坐标系
    Sophus::SE3f Twc = GetPoseInverse();
    Eigen::Matrix3f Rwc = Twc.rotationMatrix();
    Eigen::Vector3f Ow = Twc.translation();

    xLine3D.first  = Rwc * Xc1 + Ow;
    xLine3D.second = Rwc * Xc2 + Ow;

    // 4. 获取颜色信息（原图端点处）
    // 注意：imgLeftRGB 在 KeyFrame 创建后不会被修改，读取是安全的
    if(imgLeftRGB.empty())
        return false;

    int u1 = static_cast<int>(std::round(kl.startPointX));
    int v1 = static_cast<int>(std::round(kl.startPointY));
    int u2 = static_cast<int>(std::round(kl.endPointX));
    int v2 = static_cast<int>(std::round(kl.endPointY));
...
bool KeyFrame::GetLineEndPointEigen(int lineIdx, Eigen::Vector2f& s_out, Eigen::Vector2f& e_out)
{
    ...
    if(lineIdx < mvKeyLines.size() && lineIdx >= 0)
    {
        ...
        s_out[1] = mvKeyLines[lineIdx].startPointY;
        e_out[0] = mvKeyLines[lineIdx].endPointX;
        e_out[1] = mvKeyLines[lineIdx].endPointY;
        return true;
    }
    return false;
}
