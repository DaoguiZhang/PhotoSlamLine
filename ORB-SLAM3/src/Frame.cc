...
//Pure Monocular with Line (No IMU)
Frame::Frame(const cv::Mat &imGray, const cv::Mat &imRGB, const double &timeStamp, ORBextractor* extractor, LSDextractor* lsd_extractor, ORBVocabulary* voc, GeometricCamera* pCamera, cv::Mat &distCoef, const float &bf, const float &thDepth)
    :mpcpi(NULL),mpORBvocabulary(voc),mpORBextractorLeft(extractor),mpORBextractorRight(static_cast<ORBextractor*>(NULL)), mpLineExtractorLeft(lsd_extractor), mpLineExtractorRight(static_cast<LSDextractor*>(NULL)),
     ...
     mpImuPreintegrated(NULL),mpPrevFrame(NULL),mpImuPreintegratedFrame(NULL), mpReferenceKF(static_cast<KeyFrame*>(NULL)), mbIsSet(false), mbImuPreintegrated(false), mpCamera(pCamera),
     mpCamera2(nullptr), mbHasPose(false), mbHasVelocity(false), mLineSampleStep(0.1f), mLineViewWeight(2.0f), mLineSigma(3.0f), mLineTopK(2)
{
    // Frame ID
    mnId=nNextId++;
    ...
    // Set no stereo line information
    mvuLineRight.clear();
    mvLineDepth.clear();
    for(int i = 0; i < NL; ++i)
    {
        mvuLineRight.emplace_back(std::make_pair(-1.0f,-1.0f));
        mvLineDepth.emplace_back(std::make_pair(-1.0f,-1.0f));
        mvLineDepthOpti.emplace_back(std::make_pair(-1.0f,-1.0f));
    }
...
void Frame::setLineSegmentSamplingParams(float step, float viewWeight, float sigma, int topK)
{
    mLineSampleStep = step;
    mLineViewWeight = viewWeight;
    mLineSigma = sigma;
    mLineTopK = topK;
}
...
std::vector<std::size_t> Frame::GetLinesInRegionNew(const float &x1, const float &y1, 
                                                 const float &x2, const float &y2, 
                                                 const float &r) const
{
    std::vector<std::size_t> vIndices;
    ...
    // 1. 计算预测线段的包围盒，并向外扩张搜索半径 r (Expanded Predicted Bounding Box)
    const float minX_pred = std::min(x1, x2) - r;
    const float maxX_pred = std::max(x1, x2) + r;
    const float minY_pred = std::min(y1, y2) - r;
    const float maxY_pred = std::max(y1, y2) + r;

    // 2. 将扩张后的包围盒坐标转换为网格索引范围
    const int nMinCellX = std::max(0, (int)floor((minX_pred - mnMinX) * mfGridElementWidthInv));
    const int nMaxCellX = std::min((int)FRAME_GRID_COLS - 1, (int)ceil((maxX_pred - mnMinX) * mfGridElementWidthInv));
    const int nMinCellY = std::max(0, (int)floor((minY_pred - mnMinY) * mfGridElementHeightInv));
    const int nMaxCellY = std::min((int)FRAME_GRID_ROWS - 1, (int)ceil((maxY_pred - mnMinY) * mfGridElementHeightInv));
    ...
    for(int ix = nMinCellX; ix <= nMaxCellX; ix++) {
        for(int iy = nMinCellY; iy <= nMaxCellY; iy++) {
            ...
            // 4. 遍历网格中的候选线段，进行精确的包围盒重叠测试 (AABB Collision Detection)
            for(std::size_t iL : vCell) {
                const cv::line_descriptor::KeyLine &kl = mvKeyLinesUn[iL];
                
                // 获取当前候选线段的实际包围盒 (Candidate Bounding Box)
                float minX_cand = std::min(kl.startPointX, kl.endPointX);
                float maxX_cand = std::max(kl.startPointX, kl.endPointX);
                float minY_cand = std::min(kl.startPointY, kl.endPointY);
                float maxY_cand = std::max(kl.startPointY, kl.endPointY);

                // AABB 碰撞检测核心逻辑：
                // 如果 矩形A的最小X <= 矩形B的最大X 且 矩形A的最大X >= 矩形B的最小X，说明X轴重叠
                bool overlapX = (minX_cand <= maxX_pred) && (maxX_cand >= minX_pred);
                bool overlapY = (minY_cand <= maxY_pred) && (maxY_cand >= minY_pred);

                // 只有 X 和 Y 方向的包围盒都发生重叠，才说明两条线段在空间上相近
    ...
    // 5. 直接返回。因为 AssignFeaturesToGrid 中一条线仅根据中点 hash 到一个 Grid Cell，
    // 不存在一条线被放入多个 Cell 的情况，所以绝不会有重复索引，无需去重。
    return vIndices;
}
