...
namespace ORB_SLAM3
{
    struct Data_MLEstimateLine3d_Compact
    {
        ...
        Data_MLEstimateLine3d_Compact(std::vector<RandomPoint3d>& _pts) :pts(_pts) {}

    };

    RandomLine3d extract3dline( std::vector<cv::Point3d>& pts, std::ofstream &origPoints,
                            std::ofstream &optiPoints)
    ...
    // output: inlier points, line parameters: midpt and direction
    {
        ...
        std::vector<cv::Point3d> maxInlierSet;

        cv::Point3d bestA, bestB;
        //一堆点中，找出来直线的两端点
        for(int iter=0; iter<maxIterNo;iter++) {
            std::vector<cv::Point3d> inlierSet;
            ...
            */
            ...
            std::cout<<"dist takes "<<dist<< std::endl;
            if (dist<distThresh) {
        ...
        std::cout<<"Lineextractor:point number of rl:"<<rl.pts.size()<<std::endl;
        ...
        if (maxInlierSet.size() >= 2) 
        {
            ...
            while(true) {
                std::vector<cv::Point3d> tmpInlierSet;
            ...
            *
            * */
            std::cout<<rl.A.x<<",y"<<rl.A.y<<",z"<<rl.A.z<<"; x="<<rl.B.x<<",y= "<<rl.B.y<<",z="<<rl.B.z<<std::endl;
    ...
    //使用TLS来求解直线方程
    void computeLine3d_svd_tls(std::vector<cv::Point3d> pts, cv::Point3d & mean, cv:: Point3d& drct)
    {
        int n = pts.size();
    ...
    // compute msld and gradient
    {
        ...
        std::vector<std::vector<double> > GDM; //GDM.reserve(2*(int)len);
        double step = 1; // the step length between sample points on line segment
        for (int i=0; i*step < len; ++i) 
        {
            std::vector<double> col; col.reserve(9);
            //	col.clear();
    ...
    bool get_pt_3d (cv::Point2d p2, cv::Point3d& p3, const cv::Mat& depth)
    {
        ...
        if((floor(p2.x) == p2.x) && (floor(p2.y) == p2.y)) {// boundary issue
            col = std::max(int(p2.x-1),0);
            row = std::max(int(p2.y-1),0);
        } else {
            col = int(p2.x);
            row = int(p2.y);
        }
    ...
    //	MyTimer timer; 	timer.start();
        ...
        if(idx_end1 > idx_end2) std::swap(idx_end1, idx_end2); // ensure idx_end1 < idx_end2
        ...
        data.idx1 = idx_end1;
        data.idx2 = idx_end2;
        std::vector<double> paraVec, measVec;
        for(int i = 0; i < line.pts.size(); ++i) {
            measVec.push_back(0);
    ...
    std::vector<int> computeRelativeMotion_Ransac (std::vector<RandomLine3d> a, std::vector<RandomLine3d> b, cv::Mat& Ro, cv::Mat& to)
    ...
    // ransac
    {
        ...
        std::vector<std::vector<double> > aA(a.size()), aB(a.size()), aAB(a.size()), bAB(a.size());
        ...
        int iter = 0;
        std::vector<int> maxConSet;
        cv::Mat bR, bt;
        while(iter<maxIters) {
            std::vector<int> inlier;
            iter++;
            random_unique(indexes.begin(), indexes.end(),minSolSetSize);// shuffle
            std::vector<RandomLine3d> suba, subb;
        ...
        std::vector<RandomLine3d> ina, inb;
        ...
        cv::Mat R = Ro, t = to;
        std::vector<int> prevConSet;
        while(1) {
            std::vector<int> conset;
            for(int i=0; i<a.size(); ++i) {
    ...
    {
        if (pt.U.cols != 3)	{
            std::cerr<<"Error in mah_dist3d_pt_line: R matrix must be 3x3"<< std::endl;
            std::exit(0);
        }
        ...
        double s0 = std::sqrt(pt.W.at<double>(0)), s1 = std::sqrt(pt.W.at<double>(1)), s2 = std::sqrt(pt.W.at<double>(2));
    ...
    {
        if(a.size()<2)	{
            std::cerr<<"Error in computeRelativeMotion_svd: input needs at least 2 pairs!\n";
            return;
        }
        ...
        //cout<<"q="<<q<<endl;
        R = q2r(q); // find the Q2R function(to do next)
    ...
    // note: q=(a,b,c,d)=a + b i + c j + d k, where (b,c,d) is the rotation axis
    {
        ...
        double nm = std::sqrt(a*a+b*b+c*c+d*d);
        a = a/nm;
        b = b/nm;
        c = c/nm;
        d = d/nm;
    ...
    void computeLine3d_svd (const std::vector<RandomPoint3d>& pts, const std::vector<int>& idx, cv::Point3d& mean, cv::Point3d& drct)
    // input: collinear 3d points with noise
    // output: line direction vector and point
    // method: linear equation, PCA
    {
    ...
    RandomPoint3d compPt3dCov (cv::Point3d pt, cv::Mat K, double time_diff_sec)
    {
        ...
        rp.W_sqrt[0] = std::sqrt(svd.w.at<double>(0));
        rp.W_sqrt[1] = std::sqrt(svd.w.at<double>(1));
        rp.W_sqrt[2] = std::sqrt(svd.w.at<double>(2));
    ...
    // output: inlier points, line parameters: midpt and direction
    {
        ...
        int maxIterNo = std::min(10, int(pts.size()*(pts.size()-1)*0.5));
        ...
        for(int iter=0; iter<maxIterNo;iter++) {
            ...
            if(inlierSet.size() > maxInlierSet.size())	{
                std::vector<RandomPoint3d> inlierPts(inlierSet.size());
    ...
    */
        ...
        out = std::sqrt((term1 * term1 + term2* term2 + term3 * term3)/( term4*term4 + term5*term5 + term6*term6));
/*
