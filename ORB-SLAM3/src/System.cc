    ...
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false)
{
    ...
    //std::cerr << "strSettingsFile.c_str(): " << strSettingsFile.c_str() << std::endl;
    ...
    if(GetLineMode() >= 1 && GetLineLoopMode() >= 1)
        mptLoopClosing = new thread(&ORB_SLAM3::LoopClosing::RunWithLine, mpLoopCloser);
    else
...
void System::SetLineFeatrues(bool check_line)
{
    mUseLineFlag = check_line;
}

bool System::GetLineFeatrueFlag()
{
    return mUseLineFlag;
}
...
Sophus::SE3f System::TrackRGBDWithLine(const cv::Mat &im, const cv::Mat &depthmap, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename)
{
    if(mSensor!=RGBD  && mSensor!=IMU_RGBD)
    {
        cerr << "ERROR: you called TrackRGBD but input sensor was not set to RGBD." << endl;
    ...
    Sophus::SE3f Tcw = mpTracker->GrabImageRGBDWithLine(imToFeed,imDepthToFeed,timestamp,filename);
...
Sophus::SE3f System::TrackMonocularWithLine(const cv::Mat &im, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename)
{
