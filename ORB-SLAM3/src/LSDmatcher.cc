    ...
    int LSDmatcher::Fuse(KeyFrame *pKF, cv::Mat Scw, const vector<MapLine *> &vpLines, float th, vector<MapLine *> &vpReplaceLine)
{
    ...
    /**
 * @brief 找出直方图中包含元素最多的前三个 bin
 * * 在线段（或特征点）匹配中，真实的匹配往往具有一致的全局旋转变化。
 * 我们可以将匹配线段的角度差放入直方图。真实匹配会集中在极少数几个 bin 中，
 * 而误匹配的角度差通常是随机分布的。提取 Top-3 bins 可以有效保留真实匹配并剔除随机噪声。
 * *@param histo 直方图数组（每个 bin 存储属于该角度差的线段匹配索引）
 * @param L 直方图的长度（即 bin 的总数，通常是 30）
 * @param ind1 最大 bin 的索引（输出）
 * @param ind2 第二大 bin 的索引（输出）
 * @param ind3 第三大 bin 的索引（输出）
 */
void LSDmatcher::ComputeThreeMaximaNew(std::vector<int>* histo, const int L, int &ind1, int &ind2, int &ind3)
{
    // 初始化前三名的匹配数量和索引
    int max1 = -1;
    int max2 = -1;
    ...
    // 遍历整个直方图，寻找包含元素最多的前三个 bin (O(N) 时间复杂度)
    for(int i = 0; i < L; i++)
    {
        const int s = histo[i].size();
        if(s > max1)
        {
            // 如果找到新的第一名，原本的第一名退居第二，第二退居第三
            max3 = max2;
            max2 = max1;
            max1 = s;
            ind3 = ind2;
    ...
    // ==========================================
    // 启发式噪声过滤 (Heuristic Noise Filtering)
    // ==========================================
    // 如果第二名或第三名的数量远远小于第一名（例如少于 10%），
    // 则认为这些 bin 中的匹配不构成一个有效的主导方向，极有可能是随机的噪声（错配）。
    // 将它们的索引重置为 -1，这样在外层循环中，这些离散的匹配就会被当做 Outlier 剔除。
...
int LSDmatcher::SearchForInitialization(Frame &F1, Frame &F2, std::vector<cv::Point2f> &vbPrevMatched, std::vector<int> &vnMatches12, int windowSize)
{
    int nmatches = 0;
    vnMatches12 = std::vector<int>(F1.mvKeyLinesUn.size(), -1);

    const int HISTO_LENGTH = 30;
    std::vector<int> rotHist[HISTO_LENGTH];
    for(int i=0; i<HISTO_LENGTH; i++)
        rotHist[i].reserve(500);
    const float factor = HISTO_LENGTH / 180.0f; 
    ...
    for(size_t i1=0, iend1=F1.mvKeyLinesUn.size(); i1<iend1; i1++)
    {
        ...
        // ==========================================
        // Instead of searching a radius around a single midpoint, we search the bounding 
        // box of the entire line (using the tracking seed from vbPrevMatched to shift it)
        
        // Calculate how much the line has moved since F1
        float shift_x = vbPrevMatched[i1].x - ((kl1.startPointX + kl1.endPointX) * 0.5f);
        float shift_y = vbPrevMatched[i1].y - ((kl1.startPointY + kl1.endPointY) * 0.5f);

        // Shift the endpoints to the predicted location in F2
        float pred_sX = kl1.startPointX + shift_x;
        float pred_sY = kl1.startPointY + shift_y;
        float pred_eX = kl1.endPointX + shift_x;
        float pred_eY = kl1.endPointY + shift_y;
        ...
        // Calculate the midpoint of the PREDICTED line 1
        float mid_x1 = (pred_sX + pred_eX) * 0.5f;
        float mid_y1 = (pred_sY + pred_eY) * 0.5f;

        for(std::vector<size_t>::iterator vit=vIndices2.begin(); vit!=vIndices2.end(); vit++)
        {
            size_t i2 = *vit;
            cv::line_descriptor::KeyLine kl2 = F2.mvKeyLinesUn[i2];

            // ==========================================
            // 2. POINT-TO-LINE PERPENDICULAR DISTANCE
            // ==========================================
            // Calculate the infinite line equation for kl2: ax + by + c = 0
            float a = kl2.startPointY - kl2.endPointY;
            float b = kl2.endPointX - kl2.startPointX;
            float c = kl2.startPointX * kl2.endPointY - kl2.endPointX * kl2.startPointY;
            
            float norm = std::sqrt(a*a + b*b) + 1e-6f;
            
            // Perpendicular distance from kl1's midpoint to kl2's infinite line
            float perp_dist = std::fabs(a * mid_x1 + b * mid_y1 + c) / norm;
        ...
        if(bestDist <= TH_LOW)
        {
            ...
            {
                cv::line_descriptor::KeyLine kl2 = F2.mvKeyLinesUn[bestIdx2];

                // Geometry Check: Angle Difference (180-degree ambiguity)
                float angle_diff = std::fabs(kl1.angle - kl2.angle);
                ...
                if(mbCheckOrientation)
                {
                    float angle1 = kl1.angle * 180.0f / M_PI;
                    float angle2 = kl2.angle * 180.0f / M_PI;
    ...
    {
        int ind1=-1, ind2=-1, ind3=-1;
        ComputeThreeMaximaNew(rotHist, HISTO_LENGTH, ind1, ind2, ind3);
    ...
    // Update tracking seeds for the next frame
    for(size_t i1=0, iend1=vnMatches12.size(); i1<iend1; i1++)
    {
        if(vnMatches12[i1] >= 0)
        {
            cv::line_descriptor::KeyLine matched_kl = F2.mvKeyLinesUn[vnMatches12[i1]];
            float mid_x = (matched_kl.startPointX + matched_kl.endPointX) * 0.5f;
