...
#include "include/gaussian_trainer_line.h"

GaussianTrainerLine::GaussianTrainerLine()
{}

//TODO: Deprecated. Should remove this
void GaussianTrainerLine::trainingOnce(
    ...
    std::vector<int> checkpoint_iterations/*, checkpoint*/)
{
    ...
    for (int iteration = first_iter; iteration <= opt.iterations_; ++iteration) {
        ...
        std::map<std::size_t, std::shared_ptr<GaussianKeyframeLine>> viewpoint_stack; // .empty() <=> None
...
void GaussianTrainerLine::trainingReport(
    int iteration,
    int num_iterations,
    torch::Tensor& Ll1,
    torch::Tensor& loss,
